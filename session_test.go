...
//go:generate sh -c "go run github.com/golang/mock/mockgen -package webtransport -destination mock_connection_test.go github.com/lucas-clemente/quic-go Connection && sed -i '' -e 's@qerr.ApplicationErrorCode@quic.ApplicationErrorCode@g' mock_connection_test.go && goimports -w mock_connection_test.go"
//go:generate sh -c "go run github.com/golang/mock/mockgen -package webtransport -destination mock_stream_test.go github.com/lucas-clemente/quic-go Stream && sed -i '' -e 's@protocol.StreamID@quic.StreamID@g' -e 's@qerr.StreamErrorCode@quic.StreamErrorCode@g' mock_stream_test.go && goimports -w mock_stream_test.go"
